...
-- Table structure for `cart` table
CREATE TABLE `cart` (
  `cart_id` INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,  
  `user_uuid` CHAR(32) DEFAULT NULL,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `cart_uuid` CHAR(32) NOT NULL,  -- Ensure `cart_uuid` is here as a column
  UNIQUE KEY `cart_uuid` (`cart_uuid`)  -- Adding an index on cart_uuid
);
...
-- Table structure for `cartitems` table
CREATE TABLE `cartitems` (
  `cart_item_id` INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `cart_uuid` CHAR(32) NOT NULL, 
  `product_id` INT(11) NOT NULL,  -- product_id must be NOT NULL
  `quantity` INT(11) NOT NULL DEFAULT 1,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  FOREIGN KEY (`cart_uuid`) REFERENCES `cart`(`cart_uuid`) ON DELETE CASCADE,
...
CREATE TABLE users (
  user_uuid CHAR(32) NOT NULL PRIMARY KEY,  
...
--
-- Add unique key on cart_uuid instead of a primary key
ALTER TABLE `cart`
  ADD KEY `user_uuid` (`user_uuid`),  -- Index for user_uuid
  ADD COLUMN `is_locked` TINYINT(1) DEFAULT 0 AFTER `updated_at`,
  ADD COLUMN `shipping_address` VARCHAR(255) DEFAULT NULL AFTER `is_locked`;
--
...
ALTER TABLE `orders`
  ...
  ADD COLUMN `checkout_id` char(32) DEFAULT NULL AFTER `order_uuid`,
