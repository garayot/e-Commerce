...
class SellerProductController
{
    ...
    public function addNewProduct($data)
    {
        ...
        if (
            ...
            !(
                is_int($data['category_id']) ||
                ctype_digit(strval($data['category_id']))
            ) ||
            !(
                is_int($data['brand_id']) ||
                ctype_digit(strval($data['brand_id']))
            ) ||
            !(
                is_int($data['stock_quantity']) ||
                ctype_digit(strval($data['stock_quantity']))
            ) ||
            !is_string($data['size']) ||
            empty($data['size']) ||
        ...
        $stmt = $this->db->prepare(
            ...
            VALUES (:product_name, :description, :price, :stock_quantity, :category_id, :brand_id, :size, :color, :image_url, :user_uuid)"
        );
    ...
    public function getListedProducts()
    {
        ...
        $stmt = $this->db->prepare(
            "SELECT product_id, product_name, description, price, stock_quantity, image_url, created_at
            FROM products WHERE user_uuid = :user_uuid"
        );
        $stmt->bindValue(':user_uuid', $user_uuid, PDO::PARAM_STR);
        $stmt->execute();
    ...
    public function updateProduct($product_id)
    {
        ...
        $stmt = $this->db->prepare(
            'SELECT * FROM products WHERE product_id = :product_id AND user_uuid = :user_uuid'
        );
        ...
        $stmt = $this->db->prepare(
            "UPDATE products SET product_name = :product_name, description = :description, price = :price, stock_quantity = :stock_quantity, category_id = :category_id, 
            brand_id = :brand_id, size = :size, color = :color, image_url = :image_url WHERE product_id = :product_id AND user_uuid = :user_uuid"
        );
    ...
    public function deleteProduct($product_id)
    {
        ...
        $stmt = $this->db->prepare(
            'SELECT user_uuid FROM products WHERE product_id = :product_id'
        );
        $stmt->bindValue(':product_id', $product_id, PDO::PARAM_INT);
        $stmt->execute();
        $product = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    public function filterProductsByBrand($brand_name)
    {
        ...
        $stmt = $this->db->prepare(
            'SELECT * FROM products WHERE LOWER(brand_name) = :brand_name'
        );
